...
<body>
	...
	<div class="container center-box shadow-border">
		<h2 class="header-text">
			<img src="images/n3c_logo.png" class="n3c_logo_header" alt="N3C Logo">N3C Email Domain to ROR Mapping
		</h2>
		
	    <sql:query var="categories" dataSource="jdbc/N3CLoginTagLib">
	        select substring(email from '.*@(.*)'),official_institution,count(*),institutionid,institutionname from dua_master,registration as foo where enclave and not exists (select orcid_id from palantir.n3c_user as bar where foo.orcid_id = bar.orcid_id) and official_institution not in ('NIH','login.gov') and substring(email from '.*@(.*)') = substring(duacontactemail from '.*@(.*)') group by 1,2,4,5 order by 3 desc;
	    </sql:query>
	    <table>
